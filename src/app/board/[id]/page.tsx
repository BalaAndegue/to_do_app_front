...
export default function BoardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { lists, fetchLists, loading: listsLoading, error: listsError, createList, renameList, deleteList } = useLists(boardId);
  const { user } = useAuth();
  ...
  const handleCreateList = async () => {
    const ok = await createList(newListName);
    ...
    <div className="mx-auto flex w-full max-w-[100vw] flex-col gap-6 px-6 py-10">
            ...
            <div className="flex flex-col gap-2">
              {members.map(m => {
                ...
                return (
                  <div
                    key={m.id}
                    className="flex flex-wrap items-center gap-3 rounded-xl bg-white/10 px-4 py-3"
                  >
                    {/* Avatar */}
      ...
      <div className="flex gap-4 overflow-x-auto pb-4">
        ...
        <div className="flex h-fit min-w-72 flex-shrink-0 flex-col gap-2 rounded-2xl border border-[var(--line)] bg-[var(--surface)] p-4 shadow-card">
          ...
          />
          <Button onClick={handleCreateList} disabled={listsLoading || !newListName.trim()}>
            + Ajouter
          </Button>
        </div>
      </div>
